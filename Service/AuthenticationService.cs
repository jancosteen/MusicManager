...
namespace Service
{
    internal sealed class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<bool> ValiateUser(UserForAuthenticationDto userForAuth)
        {
            _user = await _userManager.FindByNameAsync(userForAuth.UserName);

            var result = (_user != null && await _userManager.CheckPasswordAsync(_user, userForAuth.Password));
            if (!result)
                _logger.LogWarn($"{nameof(ValiateUser)}: Authentication failed. Wrong userName or password.");
        ...
        {
            var principle = GetPrincipleFromExpiredToken(tokenDto.AccessToken);
